...
In a distributed system, we need a way to determine **which server or database node should store a particular key**.
...
Suppose we have **3 database nodes**.

```
DB1    DB2    DB3
```

We use a simple hash function.
...
If a new database node is added,
...
Almost every key maps to a different database.

This causes **massive data redistribution**, which is expensive and impacts system availability.

The same problem occurs when a node is removed.
...
Instead of using the modulo operator, both **nodes** and **keys** are mapped onto the same logical hash ring.
...
Hash every database node onto the ring.
...
Traverse the ring **clockwise** until the first database node is found.

That node becomes responsible for storing the key.
...
Only the keys that now fall between **DB3** and **DB4** need to move.
...
only the keys previously assigned to DB2 are reassigned to the next node in the clockwise direction.

The remaining keys are unaffected.

---

## Virtual Nodes (Recommended)
...
- Commonly used in distributed databases and caches
...
> Consistent Hashing is a partitioning technique used in distributed systems to determine which node stores a given key. Both nodes and keys are mapped onto a logical hash ring, and a key is assigned to the first node encountered in the clockwise direction. Unlike traditional hashing, adding or removing a node only redistributes a small subset of keys, making the system highly scalable and minimizing data movement.
